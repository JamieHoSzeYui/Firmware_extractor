...
romzip="$(realpath $1)"
romzipext="${romzip##*.}"
PARTITIONS="system vendor cust odm oem factory product xrom modem dtbo boot recovery tz systemex oppo_product preload_common system_ext system_other opproduct reserve india my_preload my_odm my_stock my_operator my_country my_product my_company my_engineering my_heytap my_carrier my_region my_manifest my_bigball my_version special_preload"
EXT4PARTITIONS="system vendor cust odm oem factory product xrom systemex oppo_product preload_common  my_carrier my_region my_manifest my_bigball my_version special_preload"
...
if [[ $(7z l -ba "$romzip" | grep new.dat) ]]; then
